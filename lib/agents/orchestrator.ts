...
export async function createCase(transactionData: {
  ...
  try {
    // Step 1: Create the case in MongoDB
    const caseDocument = {
    ...
    // Step 3: Trigger L1 Analyst (runs asynchronously)
    // We don't await this - let it run in the background
    // The UI will poll to see progress
    setImmediate(() => {
      runL1Analyst(transactionData.transactionId).catch((error) => {
        console.error(`❌ [Orchestrator] L1 Analyst failed:`, error);
      });
    });

    console.log(`🚀 [Orchestrator] Triggered L1 Analyst for ${transactionData.transactionId}`);
    ...
    return {
      ...
      transactionId: transactionData.transactionId,
      status: 'PROCESSING',
      message: 'Case created and L1 Analyst triggered',
    };
  } catch (error) {
